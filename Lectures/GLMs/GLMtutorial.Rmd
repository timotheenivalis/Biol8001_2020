...
We fit a simple linear model and visualise the model prediciton:
```{r}
...
* Residuals are not independent of each others (they are more or less ranked from left to right) and their variation is not constant (from left to right the variation is small, then big, then small).
...
* We see bands in the first plot: residuals are not independent of each others.
* The QQplot and the histogram show signs of non-normality (althought that is not a big problem! Linear models are pretty robust to non-normality)

*That is not a good linear model. Inference from such a model is unreliable. In particular you should not trust any measure of uncertainty (Standard Error, Confidence Interval, p-value).*
...
Before discussing what a GLM is in details, let's see that it is very easy to fit one in R. For now we can use the function `glm()`, which works exactly like `lm()`, except that you need to specify a family:
...
ANOVAs are difficult with GLMs, but we can still have a look to confirm that there is good evidence that $x$ has an effect on $y$. We just need to specify `test = LRT` or `test=Chisq`.
```{r}
anova(glm1, test = "LRT")
```
...
ggplot(survdat, aes(x = mass, y=survival)) +
...
```{r}
vole_glm <- glm(survival ~ mass, data = survdat)
summary(vole_glm)

```

So it looks like higher body mass correlates with lower survival. Do you trust this result?
