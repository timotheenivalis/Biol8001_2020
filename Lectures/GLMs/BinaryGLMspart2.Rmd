...
(p <- survdat_early %>% group_by(sex, survival) %>%
  summarize(count=n()) %>% 
...
```{r}
pred_survival <- predict(vole_s_glm, type = "response")

survdat_early$simul_surv <- sapply(pred_survival, function(x) rbinom(n = 1, size = 1, prob = x))

survdat_early

survdat_early %>% group_by(sex) %>%
  summarise(simulated_mean = mean(simul_surv), real_mean=mean(survival),
            simulated_var = var(simul_surv), real_var=var(survival))
